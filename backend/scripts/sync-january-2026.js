/**
 * ═══════════════════════════════════════════════════════════════
 *  СИНХРОНИЗАЦИЯ SAP → БД: ЯНВАРЬ 2026
 * ═══════════════════════════════════════════════════════════════
 *
 *  • Загружает все операции за 2026-01-01 — 2026-01-31
 *  • Чанки по 5 дней с retry-логикой (3 попытки, exponential backoff)
 *  • WCR-маппинг и тарифы берутся из БД (не hardcoded!)
 *  • amount = количество_АЕИ × стоимость_операции
 *  • DELETE + MERGE — идемпотентность при повторном запуске
 */
...
const PERIOD_START = new Date(Date.UTC(2026, 0, 1));                    // 2026-01-01
const PERIOD_END   = new Date(Date.UTC(2026, 0, 31, 23, 59, 59, 999)); // 2026-01-31
const CHUNK_DAYS   = 5;
const MAX_RETRIES  = 3;
...
async function main() {
  console.log('╔══════════════════════════════════════════════════════════╗');
  console.log('║  СИНХРОНИЗАЦИЯ SAP → БД: ЯНВАРЬ 2026                    ║');
  console.log('║  Период: 2026-01-01 — 2026-01-31                        ║');
  console.log('╚══════════════════════════════════════════════════════════╝\n');
  ...
  // Тарифы актуальные на январь 2026
  ...
  // ─── 2. Удаляем старые данные за январь (идемпотентность) ───
  console.log('━━━ 2. ОЧИСТКА СТАРЫХ ДАННЫХ ЗА ЯНВАРЬ ━━━');
  const delRes = await pool.request().query(`
    DELETE FROM operations
    WHERE operation_date >= '2026-01-01' AND operation_date < '2026-02-01'
  `);
  ...
  for (const wh of warehouses) {
    ...
    for (let i = 0; i < batch.length; i += MERGE_SIZE) {
      ...
      const values = chunk.map(row => {
        ...
        return `(${uid}, ${wc}, ${ot}, ${pa}, ${cnt}, ${act}, ${dt}, ${amt}, ${sap})`;
      }).join(',\n          ');
  ...
  await pool.close();
  console.log('\n✅ СИНХРОНИЗАЦИЯ ЯНВАРЯ 2026 ЗАВЕРШЕНА\n');
}
