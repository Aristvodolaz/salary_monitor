/**
 * ═══════════════════════════════════════════════════════════════
 *  СИНХРОНИЗАЦИЯ SAP → БД: ФЕВРАЛЬ 2026
 * ═══════════════════════════════════════════════════════════════
 *
 *  • Загружает все операции за 2026-02-01 — 2026-02-28
 ...
 *  • Детальный лог по складу 02DQ (Канчурина, user_id=565)
 */
...
const PERIOD_START = new Date(Date.UTC(2026, 1, 1));  // 2026-02-01
const PERIOD_END   = new Date(Date.UTC(2026, 1, 28, 23, 59, 59, 999)); // 2026-02-28
const CHUNK_DAYS   = 5;
const MAX_RETRIES  = 3;
const KANCHURINA_USER_ID = 565;
...
async function main() {
  console.log('╔══════════════════════════════════════════════════════════╗');
  console.log('║  СИНХРОНИЗАЦИЯ SAP → БД: ФЕВРАЛЬ 2026                   ║');
  console.log('║  Период: 2026-02-01 — 2026-02-28                        ║');
  console.log('╚══════════════════════════════════════════════════════════╝\n');
  ...
  const wcrMap = new Map(wcrRes.recordset.map(r => [r.wcr_code, { opType: r.operation_type, area: r.participant_area }]));
  ...
  // ─── 2. Удаляем старые данные за февраль (идемпотентность) ───
  console.log('━━━ 2. ОЧИСТКА СТАРЫХ ДАННЫХ ЗА ФЕВРАЛЬ ━━━');
  ...
  for (const wh of warehouses) {
    ...
    // Показываем пример первой записи для 02DQ
    if (wh.code === '02DQ' && allItems.length > 0) {
      const s = allItems[0];
      console.log(`  │  📝 Пример записи 02DQ:`);
      console.log(`  │     Employeeid="${s.Employeeid}" Wcr="${s.Wcr}" ZsumAmountItm="${s.ZsumAmountItm}" Who="${s.Who}"`);
    }

    // ─── Обработка записей ─────────────────────────────────
    const batch = []; // для массового INSERT
    ...
    for (const item of allItems) {
      ...
      const amount  = aeiCount * rate;  // КЛЮЧЕВАЯ ФОРМУЛА: кол-во АЕИ × стоимость операции
  ...
  // ─── 5. КАНЧУРИНА ───────────────────────────────────────────
  console.log('\n╔══════════════════════════════════════════════════════════╗');
  console.log('║  👤 КАНЧУРИНА (user_id=565, склад 02DQ)                  ║');
  console.log('╚══════════════════════════════════════════════════════════╝\n');

  // Проверяем в БД
  const kanchDbRes = await pool.request().query(`
    SELECT 
      COUNT(*) AS ops_count,
    ...
    FROM operations
    WHERE user_id = ${KANCHURINA_USER_ID}
      AND operation_date >= '2026-02-01'
      AND operation_date <  '2026-03-01'
  `);
  const k = kanchDbRes.recordset[0];
  console.log(`   Операций в БД:    ${k.ops_count}`);
  console.log(`   Рабочих дней:     ${k.work_days}`);
  console.log(`   Всего АЕИ:        ${k.total_aei || 0}`);
  console.log(`   Сумма в БД:       ${(k.total_amount || 0).toFixed(2)} руб.`);
  console.log(`   Ожидается:        37 812 руб.`);
  console.log(`   Разница:          ${(37812 - (k.total_amount || 0)).toFixed(2)} руб.\n`);

  if (k.ops_count > 0) {
    // Разбивка по типам операций
    const kByType = await pool.request().query(`
      SELECT 
        o.operation_type,
        COUNT(*) AS ops_count,
      ...
      LEFT JOIN tariffs t ON
        ...
        AND t.is_active = 1
      WHERE o.user_id = ${KANCHURINA_USER_ID}
        AND o.operation_date >= '2026-02-01'
        AND o.operation_date <  '2026-03-01'
      GROUP BY o.operation_type
      ORDER BY total_amount DESC
    `);
    console.log('   ── Разбивка по типам операций ─────────────');
    kByType.recordset.forEach(r => {
      const rate = r.rate != null ? r.rate.toFixed(2) : 'N/A';
      console.log(`   ${String(r.operation_type).padEnd(35)} AEI=${String(r.total_aei).padStart(5)} × ${rate} = ${(r.total_amount||0).toFixed(2)} руб.`);
    });

    // Проверка формулы
    const formulaCheck = await pool.request().query(`
    ...
    `);
    const bad = formulaCheck.recordset[0].bad_count;
    if (bad === 0) {
      console.log('\n   ✅ Формула верна: все записи amount = count × rate');
    } else {
      console.log(`\n   ⚠️  ФОРМУЛА: ${bad} записей с расхождением amount != count × rate`);
    }
  }

  // ─── 6. Обновляем salary_summary ────────────────────────────
  console.log('\n━━━ 6. ОБНОВЛЕНИЕ salary_summary ЗА ФЕВРАЛЬ ━━━');
  try {
    await pool.request().query(`
      DELETE FROM salary_summary
      WHERE period_start >= '2026-02-01' AND period_start < '2026-03-01'
    `);
    const insertSS = await pool.request().query(`
    ...
    `);
    console.log(`   Вставлено в salary_summary: ${insertSS.rowsAffected[0]}\n`);
  } catch (err) {
    console.error(`   ❌ Ошибка salary_summary: ${err.message}\n`);
  }

  await pool.close();
  console.log('✅ СИНХРОНИЗАЦИЯ ФЕВРАЛЯ 2026 ЗАВЕРШЕНА!\n');
}
